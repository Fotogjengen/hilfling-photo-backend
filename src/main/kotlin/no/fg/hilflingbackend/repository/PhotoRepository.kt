...
open class PhotoRepository(
  ...
  fun deletePhoto(id: UUID): Boolean {
    // Find photo to ensure it exists
    val photo = database.photos.find { it.id eq id } ?: return false

    // Check if it's an analog photo
    val analogPhoto = database.analog_photos.find { it.photo eq id }

    // If it's an analog photo, delete from analog_photos table first
    if (analogPhoto != null) {
      // Using SQL DSL for deletion
      database.delete(AnalogPhotos) {
        it.photo eq id
      }
    }

    database.delete(PhotoTagReferences) {
      it.photoId eq id
    }
