...
class SearchRepository(
  ...
  fun searchMotives(
    ...
    val sortColumn =
      when (sortField) {
        ...
        SearchSortField.DATE_UPLOADED -> Motives.dateCreated
    ...
    val orderedIds: List<UUID> =
      source
        ...
        .orderBy(direction(sortColumn, sortDirection), Motives.id.asc())
  ...
  fun searchPictures(
    ...
    val sortColumn =
      when (sortField) {
        SearchSortField.DATE_TAKEN -> Motives.date
        SearchSortField.DATE_UPLOADED -> Photos.dateCreated
        SearchSortField.MOTIVE_TITLE -> Motives.title
        SearchSortField.CATEGORY -> Categories.name
        SearchSortField.PLACE -> Places.name
      }
    ...
    val photoDtos =
      sequence
        .sorted { listOf(direction(sortColumn, sortDirection), Photos.id.asc()) }
        .drop(page * pageSize)
        .take(pageSize)
        .toList()
        .map { photo ->
  ...
  private fun motiveConditions(
    ...
    if (term.isNotBlank()) conditions += Motives.title ilike "%$term%"
  ...
  private fun applyPictureFilters(
    ...
    var seq = sequence
    if (term.isNotBlank()) seq = seq.filter { Motives.title ilike "%$term%" }
